...
export default function Page(): JSX.Element {
	...
	useEffect(() => {
		const loadDishes: () => Promise<void> = async () => {
			try {
				const result: Dish[] = await fetchDishesFromMenu();
				setDishes(result);
			} catch (err) {
				setError(
	...
	return (
		<div className='mx-auto max-w-2xl space-y-6 p-6'>
			...
			{error ? (
				<div className='rounded-md border border-red-200 bg-red-100 p-4 text-center text-red-500'>
				...
				dishes.map((dish) => <Tile key={dish.id} dish={dish} />)
			)}
		</div>
	);
}
